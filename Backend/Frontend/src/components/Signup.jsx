...
const Signup = () => {
    const[authUser,setAuthUser]=useAuth();
     const {
          ...
          //watch the password and confrim password fields 
    ...
    return (
        <>
            <div className='flex justify-center items-center h-screen'>
                <form onSubmit={handleSubmit(onSubmit)} className='border border-black px-6 w-96 py-2 rounded-md space-y-3'>
                    ...
                    {errors.confirm_password && <span className='text-red-600 text-sm font-semibold'>{errors.confirm_password.message}</span>}
                    {/*Text & Button */}
                    <div className='flex justify-between' >
                        <p >Have an account? <Link to="/login" className='text-blue-500 underline cursor-pointer ml-1'>Login</Link></p>
                        <input type='submit' className='bg-green-500 py-1 cursor-pointer px-2 text-white rounded-lg' value="Signup" />
                    </div>
                </form>
            </div>
        </>
